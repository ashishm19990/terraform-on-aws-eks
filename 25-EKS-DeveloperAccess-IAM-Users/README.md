...
# Sample Role Format: arn:aws:iam::180789647333:role/inboxable-dev-eks-nodegroup-role
# Locals Block
locals {
  configmap_roles = [
    {
...
```t
# Stop Bastion Host
Login to AWS Mgmt Console -> EC2 -> inboxable-dev-Bastion-Host -> Instance State -> Stop 
...
```t
# Set password for inboxable-dev-eksdeveloper1 user
aws iam create-login-profile --user-name inboxable-dev-eksdeveloper1 --password @EKSUser101 --no-password-reset-required

# Create Security Credentials for IAM User and make a note of them
aws iam create-access-key --user-name inboxable-dev-eksdeveloper1

# Sample Output
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws iam create-access-key --user-name inboxable-dev-eksdeveloper1
{
    "AccessKey": {
        "UserName": "inboxable-dev-eksdeveloper1",
...
## Step-17: Configure inboxable-dev-eksdeveloper1 user AWS CLI Profile and Set it as Default Profile
```t
...
aws configure --profile inboxable-dev-eksdeveloper1
...
# Set default profile
export AWS_DEFAULT_PROFILE=inboxable-dev-eksdeveloper1

# Get current user configured in AWS CLI
aws sts get-caller-identity
Observation: Should see the user "inboxable-dev-eksdeveloper1" from inboxable-dev-eksdeveloper1 profile, refer below sample output
...
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws sts get-caller-identity
{
    ...
    "Arn": "arn:aws:iam::180789647333:user/inboxable-dev-eksdeveloper1"
}
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ 
```
## Step-18: Assume IAM Role and Configure kubectl and Access Kubernetes Objects which user inboxable-dev-eksdeveloper1 has access
```t
...
aws sts assume-role --role-arn "arn:aws:iam::$ACCOUNT_ID:role/inboxable-dev-eks-developer-role" --role-session-name eksdevsession101
...
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws sts get-caller-identity
{
    ...
    "Account": "180789647333",
    "Arn": "arn:aws:sts::180789647333:assumed-role/inboxable-dev-eks-developer-role/eksdevsession101"
}
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ 
...
## Step-19: Assume IAM Role and Configure kubectl and Access Kubernetes Objects which user inboxable-dev-eksdeveloper1 don't have access
```t
# Verify aws-auth configmap
kubectl -n kube-system get configmap aws-auth -o yaml
...
## Step-21: Deploy Sample App to Dev Namespace using IAM User inboxable-dev-eksdeveloper1
- Sample App manifests are created using `YAML` and we can deploy them using `kubectl`
```t
# Change Directory 
...
# Deploy kube-manifests (YAML Format) to Dev Namespace using inboxable-dev-eksdeveloper1 user
kubectl apply -f 03-app1-kube-manifests/
Observation: 
1. inboxable-dev-eksdeveloper1 has full access to Dev Namespace as per Role and RoleBinding we have defined in c11-05-k8s-role-rolebinding.tf
2. We should see a successful creation of Kubernetes Deployment and Services

# Verify Dev Namespace resources using inboxable-dev-eksdeveloper1 user
...
## Step-23: Deploy Sample App to Dev Namespace using IAM User inboxable-dev-eksdeveloper1 - Terraform Manifests
- Sample App manifests are created using `TERRAFORM LANGUAGE` and we can deploy them using `Terraform Commands`
```t
# Verify User
aws sts get-caller-identity
...
# Deploy Terraform Manifests of Sample App to Dev Namespace using inboxable-dev-eksdeveloper1 user
terraform init
...
## Step-24: Provide S3 and DynamoDB Full Access to inboxable-dev-eks-developer-role
- We need to provide S3 and DynamoDB Access to role `inboxable-dev-eks-developer-role` to deploy Apps to Dev Namespace using user `inboxable-dev-eksdeveloper1` provided if we are using `Remote State Datasource` as S3 Bucket and for `State Locking` if we are uing DynamoDB
- In AWS Mgmt Console, go to Services -> IAM -> Roles -> inboxable-dev-eks-developer-role
...
# Verify current user configured in aws cli
aws sts get-caller-identity
Observation: It should switch back to current AWS_DEFAULT_PROFILE inboxable-dev-eksdeveloper1
...
## Step-26: Apply Changes for inboxable-dev-eks-developer-role
```t
# Change Directory 
...
## Step-27: Deploy Sample App to Dev Namespace using IAM User inboxable-dev-eksdeveloper1 - Terraform Manifests - After fixing S3 and DynamoDB Access
...
# Assume IAM Role
aws sts assume-role --role-arn "arn:aws:iam::<REPLACE-YOUR-ACCOUNT-ID>:role/<REPLACE-YOUR-ROLE-NAME>" --role-session-name eksadminsession201
aws sts assume-role --role-arn "arn:aws:iam::$ACCOUNT_ID:role/inboxable-dev-eks-developer-role" --role-session-name eksdevsession103
...
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws sts get-caller-identity
{
    ...
    "Account": "180789647333",
    "Arn": "arn:aws:sts::180789647333:assumed-role/inboxable-dev-eks-developer-role/eksdevsession103"
}
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ 
...
## Step-29: Login as inboxable-dev-eksdeveloper1 user AWS Mgmt Console and Switch Roles
...
User: arn:aws:iam::180789647333:user/inboxable-dev-eksadmin1 is not authorized to perform: eks:ListClusters on resource: arn:aws:eks:us-east-1:180789647333:cluster/*
```  
- Click on **Switch Role**
  - **Account:** <YOUR_AWS_ACCOUNT_ID> 
  - **Role:** inboxable-dev-eks-developer-role
  - **Display Name:** eksdeveloper-session201
  - Select Color: any color
- Access EKS Cluster -> inboxable-dev-eksdemo1
  - Overview Tab
  - Workloads Tab
  - Configuration Tab  
...
Remove inboxable-dev-eksdeveloper1 creds
